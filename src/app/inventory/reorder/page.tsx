...
export default function ReorderPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/inventory/reorder?leadWeeks=${leadWeeks}${mesh}`);
      if (res.ok) {
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-3 md:px-4 py-6">
        ...
        <p className="text-sm text-slate-400 mb-4">
          ...
          time it takes a new order to arrive. Orders take about 4–6 weeks — adjust the lead time below.
        </p>
        ...
        {summary && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
            ...
            <div className="bg-slate-800 rounded-lg p-3 border border-slate-700">
              <p className="text-xs text-slate-400 uppercase tracking-wider">No sales</p>
              <p className="text-2xl font-bold text-slate-300">{summary.noSales}</p>
            </div>
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {needsReorder(r) && r.suggestedQty > 0 && (
                      <span className="text-sm text-white font-semibold">Order ~{r.suggestedQty}</span>
                    )}
                  </div>
                </div>
              );
