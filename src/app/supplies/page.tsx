...
export default function SuppliesPage() {
  ...
  const handleSetQuantity = async (id: string, value: number) => {
    ...
    // Clear pending value
    setPendingQuantity((prev) => { const next = { ...prev }; delete next[id]; return next; });
  };
      ...
      <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg md:text-xl font-bold text-white">Supplies</h1>
          </div>

          <button
            onClick={() => setShowAddForm(true)}
            className="px-3 md:px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs md:text-sm font-medium flex items-center gap-2"
          >
                ...
                <div key={supply.id} className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      type="number"
                      min="0"
                      value={pendingQuantity[supply.id] ?? supply.quantity}
                      onChange={(e) => setPendingQuantity((prev) => ({ ...prev, [supply.id]: e.target.value }))}
                      onBlur={() => {
                        const val = pendingQuantity[supply.id];
                        if (val !== undefined) {
                          handleSetQuantity(supply.id, Number(val));
                        }
                      }}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
