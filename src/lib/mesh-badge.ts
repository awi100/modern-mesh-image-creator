...
// 13 = purple (Size 3 thread), 14 = zinc/slate, 16 = teal, 18 = amber (all Size 5).
...
export function meshBadgeClassDark(meshCount: number): string {
  ...
  if (meshCount === 16) return "bg-teal-900/80 text-teal-300";
...
export function meshBadgeClassLight(meshCount: number): string {
  ...
  if (meshCount === 16) return "bg-teal-500/20 text-teal-400";
